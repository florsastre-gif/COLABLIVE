...
Original file is located at
    https://colab.research.google.com/drive/1gc5EBASZiLMa8FP1UvfiaTXvPTYyzAha
"""

pip install streamlit langchain-google-genai langchain-chroma langchain-community pypdf
...
# Importaciones de LangChain (Basado en tu Colab)
...
# 3. Función de Procesamiento (El "Backend" del Colab)
# Usamos @st.cache_resource para no recargar el documento en cada click
@st.cache_resource
def procesar_documento(uploaded_file):
    # Guardar archivo temporalmente para que los Loaders lo lean
    with tempfile.NamedTemporaryFile(delete=False, suffix=f".{uploaded_file.name.split('.')[-1]}") as tmp_file:
        tmp_file.write(uploaded_file.getvalue())
        tmp_path = tmp_file.name

    # A. Loader (Selección según archivo)
...
if uploaded_file and api_key:
    ...
    if user_question:
        # --- AQUÍ OCURRE LA MAGIA DE LA TRAZABILIDAD ---

        # Paso 1: Recuperar contexto (Lo que la IA "lee")
        relevant_docs = retriever.invoke(user_question)

        # Paso 2: Mostrar Trazabilidad (Streamlit Expander)
        with st.expander("🔍 Trazabilidad: ¿Qué fragmentos leyó la IA para responder?"):
            for i, doc in enumerate(relevant_docs):
                st.markdown(f"**Fragmento {i+1}** (Página {doc.metadata.get('page', 'N/A')}):")
                st.info(doc.page_content) # Muestra el texto exacto usado

        # Paso 3: Generar Respuesta (Gemini)
        # Preparamos el contexto como un solo texto
        context_text = "\n\n".join([d.page_content for d in relevant_docs])

        # Definimos el Prompt (RAG Standard)
    ...
    st.info("👈 Sube un documento PDF o TXT en la barra lateral.")